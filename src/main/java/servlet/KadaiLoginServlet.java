...
request.getParameter("UTF-8");
		...
		String hashedPassword = GenerateHashedPw.getSafetyPassword(password, salt);
		
		// 入力されたID、ハッシュしたPWに一致するユーザを検索する
		Kadaidto kadai = KadaiDAO.login(mail, hashedPassword);
		...
		if(kadai == null) {
			...
			String view = "WEB-INF/view/loginseikou.jsp";
			RequestDispatcher dispatcher = request.getRequestDispatcher(view);
			dispatcher.forward(request, response);
		}
	}
